...
export function TextHyperlink({
    ...
    const className = "duration-75 not-sm:hover:-translate-y-1 sm:hover:translate-x-1 text-zinc-400 hover:text-white flex flex-row items-center gap-2";
        ...
        if (isRoute) {
            return (
                <Link href={path} className={className} rel="noreferrer noopener">{linkInner}</Link>
            );
        }

        return (
            <a href={path} className={className} rel="noreferrer noopener" target="_blank">{linkInner}</a>
        );
}